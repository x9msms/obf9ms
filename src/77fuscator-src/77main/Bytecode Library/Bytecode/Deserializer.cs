...
namespace geniussolution.Bytecode_Library.Bytecode
{
    public class Deserializer
    {
        ...
        public Chunk DecodeFile()
        {
            ...
            // Lua 5.1 header byte 6 is 1 for little-endian, 0 for big-endian.
            // _bigEndian is only used by Read() to decide whether the bytes must be
            // reversed, so store "file endianness differs from host endianness".
            // (was: ReadByte() == 0, which made Read() reverse every value on
            //  little-endian hosts such as Linux/x64 -> garbage decode)
